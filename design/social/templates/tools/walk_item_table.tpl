<tr>
  ...
  <td width="1">
    {if and( $item.children|count()|gt(0), is_set( $insert_child_class ) )}
      <a title="{'Add'|i18n('social_user/config')}  {$item.children[0].node.class_name} in {$item.node.name|wash()}" href="{concat('openpa/add/', $item.children[0].node.class_identifier, '/?parent=',$item.node.node_id)|ezurl(no)}"><i class="fa fa-plus"></i></a>
    {elseif and( is_set( $child_class ), is_set( $insert_child_class ) )}
      <a title="{'Add'|i18n('social_user/config')}  {$child_class.name} in {$item.node.name|wash()}" href="{concat('openpa/add/', $child_class.identifier, '/?parent=',$item.node.node_id)|ezurl(no)}"><i class="fa fa-plus"></i></a>
    {elseif is_set( $insert_child_class )|not()}
      <a title="{'Add'|i18n('social_user/config')} {$item.node.class_name} in {$item.node.name|wash()}" href="{concat('openpa/add/', $item.node.class_identifier, '/?parent=',$item.node.node_id)|ezurl(no)}"><i class="fa fa-plus"></i></a>
    {/if}
  </td>
...
{if $item.children|count()|gt(0)}
  ...
  {include name=itemtree uri='design:tools/walk_item_table.tpl' redirect_if_discarded=$redirect_if_discarded redirect_after_publish=$redirect_after_publish item=$item_child recursion=$recursion insert_child_class=is_set( $insert_child_class )}
  {/foreach}
{/if}