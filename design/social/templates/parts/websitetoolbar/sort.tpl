{set scope=global persistent_variable=hash('left_menu', false(), 'extra_menu', false(), 'show_path', true())}
{ezscript_require( array( 'ezjsc::yui3', 'ezjsc::yui3io', 'ezwtsortdd.js' ) )}
  <form id="ezwt-sort-form" method="post" action={'content/action'|ezurl}>
    ...
    {if $children}
      ...
      <table id="ezwt-sort-list" class="table table-condensed" cellspacing="0">
        <tr>
          <th class="tight"></th>
          {* Name column *}
      ...
      <div class="block">
        ...
        <div class="left">
          {if and( $priority_sorting, $node_can_edit, $children_count )}
            <input id="ezwt-update-priority" class="btn btn-primary pull-right" type="submit" name="UpdatePriorityButton" value="{'Update priorities'|i18n( 'design/standard/websitetoolbar/sort' )}" title="{'Apply changes to the priorities of the items in the list above.'|i18n( 'design/standard/websitetoolbar/sort' )}" />
        ...
        <div class="right">
          ...
          {def $sort_fields = hash(
                  6, 'Class identifier'|i18n( 'design/standard/websitetoolbar/sort' ),
          ...
          <input {if $disabled}class="btn btn-disabled"{else}class="btn btn-primary"{/if} type="submit" name="SetSorting" value="{'Set'|i18n( 'design/standard/websitetoolbar/sort' )}" title="{$title}" {$disabled} />
